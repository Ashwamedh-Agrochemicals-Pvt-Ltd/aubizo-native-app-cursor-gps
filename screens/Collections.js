...
function CollectionScreen({ navigation }) {
    ...
    const fetchTransactions = async () => {
        try {
            ...
            if (response.data && response.data.success && response.data.data && response.data.data.transactions) {
                console.log("Setting transactions:", response.data.data.transactions.length, "items");
                setTransactions(response.data.data.transactions);
            } else if (response.data && Array.isArray(response.data)) {
                // Fallback for direct array response
                console.log("Setting transactions from direct array:", response.data.length, "items");
                setTransactions(response.data);
            } else if (response.data && response.data.results) {
                // Fallback for paginated response
                console.log("Setting transactions from results:", response.data.results.length, "items");
    ...
    const renderTransactionCard = ({ item }) => (
        <TouchableOpacity onPress={() => handleCardPress(item.id)}>
            <View style={styles.card}>
                ...
                <Text style={styles.ownerName}>{item.payment_info?.payment_number || item.transaction_number}</Text>
                ...
                <View style={styles.amountRow}>
                    ...
                    >
                        ...
                        >
                            {item.status_display || item.status?.charAt(0).toUpperCase() + item.status?.slice(1)}
                        </Text>
                    </View>
                </View>

                <Text style={styles.paymentMethod}>
                    {item.payment_method_info?.name || 'N/A'}
                </Text>
            </View>
        </TouchableOpacity>
    );
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top }]}>
            <StatusBar backgroundColor={DESIGN.colors.background} barStyle="dark-content" />
                    ...
                    ListEmptyComponent={() => (
                        <View style={styles.emptyContainer}>
                            ...
                            <Text style={styles.emptySubtext}>
                                {showSearch ? "Try adjusting your search" : "Create your first payment"}
                            </Text>
                        </View>
                    )}
                />
