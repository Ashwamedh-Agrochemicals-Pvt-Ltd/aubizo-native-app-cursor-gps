...
function PaymentMethodPicker({ value, onChange }) {
    ...
    const fetchPaymentMethods = async () => {
        try {
            setLoading(true);
            const response = await apiClient.get("/payment/utils/payment-methods/active/");
                ...
                <View style={styles.dropdownWrapper}>
                    ...
                    >
                        <Text style={value ? styles.text : styles.placeholderText}>
                            {value ? (value.name || 'Selected Method') : "Select Payment Method"}
                        </Text>
                    </TouchableOpacity>
                    {showMethodList && (
...
function BankDetailsInput({ paymentMethod, value, onChange }) {
    const showBankFields = paymentMethod && ['cheque', 'neft', 'rtgs', 'imps'].includes(paymentMethod.method_type);
    const showUPIFields = paymentMethod && ['upi', 'digital'].includes(paymentMethod.method_type);
            ...
            {showBankFields && (
                <>
                    ...
                    <TextInput
                        ...
                        onChangeText={(text) => onChange({ ...value, account_number: text })}
                        keyboardType="numeric"
                    />
                    {paymentMethod.method_type === 'cheque' && (
                        <>
                            <TextInput
                                ...
                                onChangeText={(text) => onChange({ ...value, cheque_number: text })}
                            />
                            <TextInput
                                style={styles.input}
                                placeholder="Cheque Date (YYYY-MM-DD)"
                                value={value.cheque_date || ""}
                                onChangeText={(text) => onChange({ ...value, cheque_date: text })}
                            />
                        </>
                    )}
                </>
            ...
            {showUPIFields && (
                <>
                    ...
                    <TextInput
                        ...
                        onChangeText={(text) => onChange({ ...value, beneficiary_name: text })}
                    />
                </>
            )}
        </View>
...
export default function CollectionForm({ onPaymentSuccess: onPaymentSuccessProp }) {
    ...
    const fetchDealerInvoices = async (dealerId) => {
        try {
            ...
            const response = await apiClient.get(`/payment/invoices/dealer/${dealerId}/`);
            ...
            if (response.data?.success && response.data?.data) {
                ...
                const invoices = response.data.data.payments || response.data.data.invoices || response.data.data || [];
    ...
    const requestPermissions = async () => {
        try {
            ...
            if (needsCameraPermission || needsMediaPermission) {
                ...
                Alert.alert(
                    'Permission Required',
                    'This app needs access to your camera and photo library to upload receipt images. This helps you attach proof of payment to your transactions.',
                    [
                        {
                            text: 'Cancel',
                            style: 'cancel'
                            ...
                            onPress: async () => {
                                try {
                                    ...
                                    if (needsMediaPermission) {
                                        permissionRequests.push(ImagePicker.requestMediaLibraryPermissionsAsync());
                                    }
                                    ...
                                    if (allGranted) {
                                        ...
                                        Alert.alert(
                                            'Permission Denied', 
                                            'Some permissions were denied. You can still upload documents, but camera and photo library features will be limited. You can change permissions in your device settings.'
                                        );
                                    }
                                } catch (error) {
    ...
    const showFileOptionsAfterPermission = async () => {
        try {
            ...
            if (mediaLibraryPermission.granted) {
                options.splice(-1, 0, { text: 'Choose from Gallery', onPress: pickImage });
            }
        ...
        try {
            ...
            if (selectedFile) {
                ...
                formData.append('attachment', {
                    ...
                    name: selectedFile.name || `receipt_${Date.now()}.${selectedFile.uri.split('.').pop()}`
                });
            ...
            // Handle documented response structure
                ...
                const successMessage = response.data?.message || `Payment of ₹${paymentAmount} recorded successfully`;
                Alert.alert(
                    "Success",
                    successMessage,
                    [{ text: "OK", onPress: () => {
                ...
                console.log("Unexpected response format, but checking if payment processed...");
    ...
    return (
        ...
        >
            <FlatList
                ...
                contentContainerStyle={[styles.contentContainer, { paddingBottom: insets.bottom + DESIGN.spacing.xl }]}
                showsVerticalScrollIndicator={false}
        ...
        <View style={styles.section}>
            <View style={styles.sectionHeader}>
                ...
                {(data.selectedDealer || data.selectedInvoice) && (
                    <TouchableOpacity style={styles.resetButton} onPress={data.resetWorkflow}>
            ...
            {showDealerList && dealers.length > 0 && (
                <View style={[
                    styles.list, 
                    dealers.length > 2 ? styles.scrollableDealerList : { maxHeight: MAX_DROPDOWN_HEIGHT }
                ]}>
                    <FlatList
                        data={dealers}
                        keyExtractor={(item) => (item.id || Math.random()).toString()}
                        ...
                        scrollEnabled={dealers.length > 2}
                        showsVerticalScrollIndicator={dealers.length > 2}
                        nestedScrollEnabled={true}
                    />
                </View>
            )}

            {showDealerList && dealers.length === 0 && !loadingDealer && dealerDisplayValue.length >= 2 && (
...
const DealerInvoicesSection = ({ data }) => {
    const { dealerInvoices, selectedInvoice, setSelectedInvoice, loadingInvoices, selectedDealer } = data;
        ...
        >
            ...
            <View style={styles.invoiceDetails}>
                <Text style={styles.invoiceAmount}>
                    {`₹${Number(invoice.pending_amount || invoice.outstanding_amount || invoice.amount || 0).toLocaleString()}`}
                </Text>
        ...
        <View style={styles.section}>
            ...
            {loadingInvoices ? (
                ...
                <FlatList
                    ...
                    style={dealerInvoices.length > 2 ? styles.scrollableInvoiceList : null}
                    scrollEnabled={dealerInvoices.length > 2}
                    showsVerticalScrollIndicator={dealerInvoices.length > 2}
                    nestedScrollEnabled={true}
                />
            ) : (
                <View style={styles.noResultsContainer}>
                    <Text style={styles.noResultsText}>No outstanding invoices found</Text>
                </View>
            )}
        </View>
    );
        ...
        <View style={styles.section}>
            ...
            <View style={styles.selectedInvoiceInfo}>
                <Text style={styles.selectedInvoiceText}>
                    {`Selected: ${(selectedInvoice.payment_number && selectedInvoice.payment_number.trim()) ||
                        (selectedInvoice.invoice_number && selectedInvoice.invoice_number.trim()) ||
                        (selectedInvoice.number && selectedInvoice.number.trim()) ||
                        ('Invoice ' + selectedInvoice.id)}`}
                </Text>
                <Text style={styles.selectedInvoiceAmount}>
                    {`Pending: ₹${(selectedInvoice.pending_amount || selectedInvoice.outstanding_amount || selectedInvoice.amount || 0).toString()}`}
                </Text>
            </View>
            ...
            <AmountInput
                ...
                maxAmount={parseFloat(selectedInvoice.pending_amount || selectedInvoice.outstanding_amount || selectedInvoice.amount || 0)}
...
const styles = StyleSheet.create({
    ...
    scrollableDealerList: {
        maxHeight: 200, // Limit height when more than 2 items
    },
